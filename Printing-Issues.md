...
This is a document dedicated to issues arising when trying to print the
[NeatoCal](https://github.com/abetusk/neatocal) single page calendar.
...
Printing is a done through the browsers print driver, which means it's
independent of any parameters explicitly set by NeatoCal.
...
This can happen when changing parameters that affect the calendar height, such
as changing the `cell_height` parameter.

The reasons it could spill onto multiple pages could be complicated so there
might not be an easy, catch-all fix.

Some strategies to try to fit it on a single page are:

- Try to adjust the `scale` parameter in the print settings
- Try to use another browser (Firefox, Safari) if the scale parameter isn't
  working in Chrome

If Chrome is absolutely necessary, there is a fix to set the CSS `zoom` property
globally by changing it manually from Chrome DevTools. This can be done by:
